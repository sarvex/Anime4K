...
def flag_simplify(layer, simplify_threshold):
    ...
    if (
        len(layer._outbound_nodes) <= simplify_threshold
        and len(layer._outbound_nodes) > 0
        and any(
            name in layer.name
            for name in ["add", "concatenate", "crelu", "up_sampling2d"]
        )
    ):
...
def parse_inputs(node):
    ...
    for name, new_node in node:
        ...
        if type(new_node) is int:
            ...
            if type(input_nodes) is list and len(input_nodes) > 0:
                ...
                if type(input_nodes[0]) is list:
                    for u in range(len(input_nodes[0])):
                        tl = [input_nodes[v][u] for v in range(len(input_nodes))]
                        templist.append(("add", tl))
                else:
                    templist.extend(("add", n) for n in input_nodes)
            newlist.append(templist)
            ...
            if type(input_nodes) is list and len(input_nodes) > 0:
                ...
                if type(input_nodes[0]) is list:
                    for u in input_nodes:
                        templist.extend(("pos_relu", v) for v in u)
                        templist.extend(("neg_relu", v) for v in u)
                else:
                    templist.extend(("pos_relu", u) for u in input_nodes)
                    templist.extend(("neg_relu", u) for u in input_nodes)
            else:
                templist.extend((("pos_relu", input_nodes), ("neg_relu", input_nodes)))
            newlist.append(templist)
    ...
    return newlist[0] if len(newlist) == 1 else newlist
...
def get_bind_num_features(node):
    if type(node) is list:
        return sum(get_bind_num_features(n) for n in node)
    name, subnode = node
    if name == "direct":
        return subnode[1]
    elif name == "add":
...
def get_binds_split(node, max_bind_num=16):
    ...
    for i in range(len(node)):
        ...
        real_max_bind_num = max_bind_num if not splits else max_bind_num - 1
        if (i + 1) == len(node):
...
def parse_expression_recur(node, with_offset=False):
    ...
    if name == "add":
        ...
        return f"(max({parse_expression_recur(subnode, with_offset)}, 0.0))"
        ...
        if with_offset:
            return f"({subnode[0]}_texOff(vec2(x_off, y_off)))"
        else:
            return f"({subnode[0]}_tex({subnode[0]}_pos))"
...
def parse_expression(node, feature_no, with_offset=False):
    if with_offset:
        return f"#define go_{str(feature_no)}(x_off, y_off) {parse_expression_recur(node, with_offset)}"
    else:
        return f"#define g_{str(feature_no)} {parse_expression_recur(node, with_offset)}"
...
def parse_conv2d(layer, hook="MAIN", desc=None, when=None, dtype=np.float32, max_bind_num=16):
    ...
    for n in range(0, output_size, 4):
        ...
        for bi, binds in enumerate(bind_split_list):
            ...
            bind_names = list(get_unique_binds(binds))
            ...
            shader_string += (
                f"//!DESC {desc}-Conv-{str(min(weights.shape[3], 4))}x{str(weights.shape[0])}x{str(weights.shape[1])}x{str(weights.shape[2])}"
                + "\n"
            )
            shader_string += f"//!HOOK {hook}" + "\n"
            if "lastresid" in layer.name:
                shader_string += f"//!BIND {current_bind_name}" + "\n"

            for bind_name in bind_names:
                shader_string += f"//!BIND {bind_name}" + "\n"
            if bi > 0:
                shader_string += f"//!BIND {current_bind_name}" + "\n"

            shader_string += f"//!SAVE {current_bind_name}" + "\n"
